def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    def nested_list_copyer(nested_list):
        """
        return a copy of the given nested_list
        """
        new_list = []
        for lst in nested_list:
            new_list.append(list(lst))
        return new_list
    new_list = nested_list_copyer(nested_list)
    for lst in new_list:
        lst.insert(0, item)
    return new_list
...
def trade(first, second):
    ...
    while len(first) > m and len(second) > n and not equal_prefix():
    ...
    equal_prefix = lambda: sum(first[:m]) == sum(second[:n])
    while (first[m:] or second[n:]) and not equal_prefix():
        if first[m:] and first[:m] < second[:n]:
            m += 1
        elif second[n:] and first[:m] > second[:n]:
            n += 1
        else:
            break
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    full_score, obtained_score = 0, 0
    def glookup(assignment, grade):
        nonlocal full_score, obtained_score
        full_score += cs61a[assignment]
        obtained_score += grade
        return obtained_score / full_score
    return glookup
