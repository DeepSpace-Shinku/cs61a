...
def make_withdraw(balance, password_set):
    ...
    attempt = 0
    wrong_passwords = []
    def withdraw(amount, password_check):
        nonlocal balance, attempt
        if attempt == 3:
            return "Too many incorrect attempts. Attempts: ['{}', '{}', '{}']".format(wrong_passwords[0], wrong_passwords[1], wrong_passwords[2])
        elif password_check == password_set:
            if amount > balance:
                return 'Insufficient funds'
            balance = balance - amount
            return balance
        else:
            wrong_passwords.append(password_check)
            attempt += 1
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    def joint(amount, password_check):
        if password_check == new_pass:
            password_check = old_pass
        return withdraw(amount, password_check)
...
def remainders_generator(m):
    ...
    11
    """
    # remainders_four = remainders_generator(4) = generator of remainders_generator
    # list(remainders_four) = [generator(0), generator(1), generator(2), generator(3)]
    # gen = generator(0)
    "*** YOUR CODE HERE ***"
    def generator(x):
        j = 1
