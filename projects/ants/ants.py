...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while search_place is not beehive:
            if search_place.bees and self.min_range <= distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def attack_all_bees(self, amount):
        """
        attack all bees in the place of SELF
        """
        bee_amount = len(self.place.bees)
        pos = 0
        for _ in range(bee_amount):
            bee = self.place.bees[pos]
            Insect.reduce_armor(bee, amount)
            if bee.place is None:
                pos -= 1
            pos += 1
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        self.attack_all_bees(amount)
        if self.armor - amount == 0:
            self.attack_all_bees(self.damage)
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.true_queen is not self and QueenAnt.true_queen is None:
    ...
    def action(self, gamestate):
        ...
        if QueenAnt.true_queen is not self:
            ...
            while True:
                ...
                if place_behind is not None:
                    if place_behind.ant and not place_behind.ant.enhanced:
                        place_behind.ant.damage *= 2
                        place_behind.ant.enhanced = True
                else:
                    break
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.armor - amount <= 0 and QueenAnt.true_queen is self:
            bees_win()
        else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.forward:
            destination = self.place.exit
        elif self.plance.exit is not gamestate.behive:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        "*** YOUR CODE HERE ***"
        FireAnt.attack_all_bees(self, self.damage)
        # END Problem Optional 1
