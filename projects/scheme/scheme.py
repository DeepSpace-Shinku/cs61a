...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        def scheme_eval_in_present_env(expr):
            return scheme_eval(expr, env)
        fcn = scheme_eval(expr.first, env)
        validate_procedure(fcn)
        evaled_operands = expr.rest.map(scheme_eval_in_present_env)
        return scheme_apply(fcn, evaled_operands, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    >>> x
    2
    >>> a = read_line("((define x 2) x)")
    >>> a
    Pair(Pair('define', Pair('x', Pair(2, nil))), Pair('x', nil))
    >>> eval_all(a, create_global_frame())
    2
    """
    # BEGIN PROBLEM 7
...
def do_define_form(expressions, env):
    ...
    >>> do_define_form(a, env)
    'f'
    >>> scheme_eval(read_line("(f 3)"), env)
    5
    >>> a = read_line('((square x) (* x x))')
    >>> b = read_line('((x) (* x x))')
    >>> a
    Pair(Pair('square', Pair('x', nil)), Pair(Pair('*', Pair('x', Pair('x', nil))), nil))
    >>> b
    Pair(Pair('x', nil), Pair(Pair('*', Pair('x', Pair('x', nil))), nil))
    """
    ...
    if scheme_symbolp(target):
        ...
        lambda_expressions = Pair(expressions.first.rest, expressions.rest)
        lambda_form = do_lambda_form(lambda_expressions, env)
        env.bindings[name] = lambda_form
        return name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    >>> do_quote_form(read_line("''hello"), env)
